...
public class Servidor2 {
	...
	public static void main(String[] args) throws Exception {
		...
		servidorPrincipal = false;
		new Servidor2().executa();
	}

	public Servidor2() {
		Servidor2.listaClientes = new ArrayList<String>();
	}

	public void executa() throws IOException 
	{
		Servidor2.servidor = new ServerSocket(Globals.PORTA);
		servidor.getInetAddress();
		meuIP = InetAddress.getLocalHost().toString().replaceAll(".*/", "");
		System.out.println("SERVIDOR SECUNDARIO ABERTO");
		...
		new Thread() {
			@Override
			public void run() {
				verificaClientes();//VERIFICA CONEXAO DOS CLIENTES
			}
		}.start();
	}

	public static void verificaClientes() 
	{
		while (true) 
		{
			try 
				...
				for (int i = 0; i < listaClientes.size(); i++) 
				{
					try 
					{
						...
						if (!ping.isReachable(Globals.atraso/50) && Globals.ping()) 
						{
							...
							{
								System.out.println("Cliente: " + listaClientes.get(i) + " caiu !!");
		...
		while (true)
		{
			try 
			{
				if (servidorPrincipal) 
				{
					...
					//VERIFICA SE O OUTRO SERVIDOR ESTA DESATUALIZADO E SE TEM CONEXAO
					if (desatualizado && ping.isReachable(Globals.atraso/50)) 
					{
						//ENVIA O BACKUP ATUALIZADO PARA O OUTRO SERVIDOR
						try 
						{
					...
					if (filaAtualizacoes.size() > 0) 
					{
						...
						try 
						{
							...
							Globals.enviarMensagem(meuIP, Globals.servidorSecundario, Globals.ENVIAMUDANCA, Globals.PORTA,alteracoes);
						} 
						catch (Exception e){
						...
						System.out.println("\t" + atualizacao.getQuantidade() + " ALTERACOES ENVIADAS PARA OS " + listaClientes.size() + " CLIENTES!");
					}
				}
				...
				if (!listaClientes.contains(cliente.getInetAddress().getHostAddress()))
				{
					...
					if (!cliente.getInetAddress().getHostAddress().equals(Globals.servidorBackup)&& !cliente.getInetAddress().getHostAddress().equals(Globals.servidorPrimario))
					{
						...
						try 
						{
							...
							System.out.println("ENVIADO BACKUP DE PONTOS PARA O NOVO CLIENTE CONECTADO...");
						} catch (Exception e) {
							// e.printStackTrace();
						}
					}
